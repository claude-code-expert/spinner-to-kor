...
## M3 — v1.2 메시지 매핑 확장 (**완료 — 2026-07-02, v1.2.0**)

**목표**: Claude Code 버전 변화에 대한 자가 적응력 + 사용자 취향 반영.

| # | 태스크 | 요구사항 | 내용 | 상태 |
|---|---|---|---|---|
| T1 | 신규 verb 감지 도구 | FR-31 | `src/detect-verbs.py` — 양패턴(JSON+NUL) 교집합 gerund 후보 → 매핑 diff 리포트. 단일 패턴 등장(일반 문자열)·비gerund·"Doing" 제외 | ✅ 완료 (테스트 10건) |
| T2 | auto-patch 경보 통합 | FR-31 | 패치 성공 시 감지 실행, 미매핑 발견 시 로그 `WARN unmapped=N` | ✅ 완료 (셸 회귀 테스트) |
| T3 | 커스텀 매핑 오버레이 | FR-32 | `~/.claude/spinner-map.json` — pools/overrides, 자동 패딩, 위반·오타·깨진 JSON → 패치 거부·무변경 | ✅ 완료 (테스트 5건) |
| T4 | 스타일 프리셋 | FR-33 | `--style semantic\|witty` + env — 위트 178개 `WITTY_RAW` 데이터화, 패딩 자동 계산 | ✅ 완료 (전수 byte 검증) |
| T5 | MAPPING.md 갱신 | NFR-07 | 커스텀·스타일·감지 절차 반영 | ✅ 완료 |

**릴리스 기준** ✅: 미래 verb 추가 가정 fixture에서 "감지(WARN) → 리포트 → 매핑 갱신 지점 안내" 동작 검증.
